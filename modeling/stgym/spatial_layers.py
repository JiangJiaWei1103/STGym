...
class ChebGraphConv(nn.Module):
    ...
    def __init__(self, in_dim: int, h_dim: int, cheb_k: int, bias: bool = True) -> None:
        ...
        self.weight = nn.Parameter(torch.FloatTensor(cheb_k, in_dim, h_dim))
...
class NAPLConvLayer(nn.Module):
    ...
    def forward(self, x: Tensor, node_emb: Tensor, As: Tensor) -> Tensor:
        ...
        Shape:
            ...
            x_conv: (B, N, h_dim)
        """
        weights = torch.einsum('nd,dkio->nkio', (node_emb, self.weights_pool))   # (N, cheb_k, in_dim, out_dim)
        bias = torch.matmul(node_emb, self.bias_pool)    # (N, out_dim)
