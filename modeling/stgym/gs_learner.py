...
class MegaCRNGSLearner(nn.Module):
    ...
    def forward(self, Memory: Tensor, We1: Tensor, We2: Tensor) -> Tensor:
        """Forward pass.
        ...
        Shape:
            As: (2, N, N)
        """
        src_emb = torch.matmul(We1, Memory)
        tgt_emb = torch.matmul(We2, Memory)
...
class GTSGSLearner(nn.Module):
    ...
    def __init__(self, n_series: int, node_features: Tensor, temperature: int) -> None:
        ...
        conv_out_dim = 16
        emb_dim = 100
        fc_in_dim = ((node_features.shape[0] - 2 * (kernel_size - 1)) * conv_out_dim)
        ...
        self.rel_rec = torch.FloatTensor(rel_rec).to(node_features.device)
        self.rel_send = torch.FloatTensor(rel_send).to(node_features.device)
