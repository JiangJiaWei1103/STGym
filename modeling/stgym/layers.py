...
class AGCGRU(nn.Module):
    ...
    """
    def __init__(self, in_dim: int, h_dim: int, emb_dim: int, cheb_k: int = 2) -> None:
        ...
        # Model blocks
        cat_dim = in_dim + h_dim
        self.gate = NAPLConvLayer(
            in_dim=cat_dim, h_dim=h_dim * 2, emb_dim=emb_dim, cheb_k=cheb_k
        )
        self.candidate_act = NAPLConvLayer(
            in_dim=cat_dim, h_dim=h_dim, emb_dim=emb_dim, cheb_k=cheb_k
        )

    def forward(self, x: Tensor, node_emb: Tensor, As: Tensor, h_0: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        """Forward pass.
        ...
        for t in range(in_len):
            ...
            gate = F.sigmoid(self.gate(torch.cat([h_prev, x_t], dim=-1), node_emb, As))
            r_t, u_t = torch.split(gate, self.h_dim, dim=-1)  # (B, N, h_dim)
            c_t = F.tanh(self.candidate_act(torch.cat([r_t * h_prev, x_t], dim=-1), node_emb, As))
            h_t = u_t * h_prev + (1 - u_t) * c_t  # (B, N, h_dim)
