    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if self.Ko > 1:
            ...
            x = self.relu(x)
            x = self.fc2(x).permute(0, 3, 1, 2) # (B, C, T, N)
...
class _ChebGraphConv(nn.Module):
    ...
    def forward(
        ...
        Shape:
            x: (B, C, T, N)
            cheb_graph_conv: (B, T, N, C')
        """
        x = torch.permute(x, (0, 2, 3, 1))  # (B, T, N, C)
...
class _GraphConvLayer(nn.Module):
    ...
    def __init__(
        ...
        if self.graph_conv_type == 'cheb_graph_conv':
            self.cheb_graph_conv = _ChebGraphConv(c_out, c_out, Ks, gso, bias)
        elif self.graph_conv_type == 'graph_conv':
            self.graph_conv = _GraphConv(c_out, c_out, gso, bias)
    ...
    def forward(
        ...
        x_gc = x_gc.permute(0, 3, 1, 2)     # (B, C', T, N)
...
class _STConvBlock(nn.Module):
    ...
    def __init__(
        ...
        self.graph_conv = _GraphConvLayer(graph_conv_type, channels[0], channels[1], Ks, gso, bias)
    ...
    def forward(
        ...
        x = self.relu(x)
        x = self.tmp_conv2(x)
        x = self.tc2_ln(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)  # (B, T, N, C') -> (B, C', T, N)
        x = self.dropout(x)
