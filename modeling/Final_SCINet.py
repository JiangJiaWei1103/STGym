...
class SCINet(nn.Module):
    def __init__(
        ...
        Parameters:
            net_params: hyperparameters of SCINet
            loss_params: hyperparameters of loss
    ...
    def eval_loss(
        ...
        if self.dataset_name in MTSFBerks:
            ...
            if self.stacks == 2:
                loss += criterion(Midoutput[:, -1, :], y_true)
        else:
            loss = criterion(y_pred, y_true)
            if self.stacks == 2:
                loss += criterion(Midoutput, y_true)
