...
class _SpatioTemporalEmbedding(nn.Module):
    ...
    Parameters:
        ...
        use_bias: whether to use bias in Fully Connected layers
    """
    ...
    def forward(
        ...
        Shape:
            SE: (N, D)
            TE: (B, t_window + out_dim, 2)
            output: (B, t_window + out_dim, N ,D)
        ...
        TE = TE.unsqueeze(dim = 2)  # (B, t_window + out_dim, 1, n_tids + 7)
