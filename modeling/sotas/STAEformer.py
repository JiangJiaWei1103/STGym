...
from modeling.stgym.layers import STAEAttentionLayer
from modeling.stgym.common_layers import AuxInfoEmbeddings
...
class STAEformer(nn.Module):
    ...
    def forward(self, x: Tensor, As: List[Tensor] = None, **kwargs: Any) -> Tuple[Tensor, None, None]:
        ...
        if self.tid_emb_dim > 0:
            tid = (x[..., 1] * self.n_tids).int()
        if self.diw_emb_dim > 0:
            diw = (x[..., 2] * N_DAYS_IN_WEEK).int()
        x = x[..., :self.in_dim]
        ...
        # Auxiliary information embeddings
        features = [x]
        x_node, _, x_tid, x_diw, x_adp = self.aux_info_emb(tid=tid, diw=diw)
