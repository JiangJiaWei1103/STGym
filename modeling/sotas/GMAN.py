...
from modeling.stgym.common_layers import Linear2d, SpatioTemporalEmbedding, AttentionLayer
from modeling.stgym.layers import GMANAttentionBlock
...
class GMAN(nn.Module):
    ...
    def __init__(
        ...
        self.spatial_emb = torch.Tensor(aux_data[0])
        ...
        self.output = nn.Sequential(
            ...
            nn.BatchNorm2d(h_dim),
            nn.ReLU(),
            Linear2d(h_dim, out_dim),
            nn.BatchNorm2d(out_dim)
        )

    def forward(
        self, x: Tensor, As: Optional[List[Tensor]] = None, ycl: Tensor = None, **kwargs: Any
        ...
        # Spatio Temporal Embedding
        st_emb = self.st_embedding(self.spatial_emb.to(x.device), temporal_emb, self.n_tids)
